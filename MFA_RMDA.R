...
function (base, group, type = rep("s", length(group)), excl = NULL, 
                                                                      ...
                                                                      2), tab.comp = NULL) 
{
  ...
  if (is.null(name.group)) 
    name.group <- paste0("Gr", 1:nbre.group)
  ...
  if (!is.null(ind.quali)) {
    ...
    if (is.null(data.group.sup)) 
      aux_quali_sup_indice <- (ncol(data) + 1):(ncol(data) + 
                                                  ncol(aux_quali_sup))
    else aux_quali_sup_indice <- (ncol(data) + ncol(data.group.sup) + 
                                    1):(ncol(data) + ncol(data.group.sup) + ncol(aux_quali_sup))
  ...
  res.globale <- PCA(data.pca, scale.unit = FALSE, col.w = ponderation, 
                     row.w = row.w, ncp = ncp, ind.sup = ind.sup, quali.sup = aux_quali_sup_indice, 
                     quanti.sup = data.group.sup.indice, graph = FALSE)
  if ((!is.null(tab.comp)) & (any("n" %in% type))) {
    if (!is.null(type.var %in% "quali")) {
      if (any(type.var %in% c("freq", "quanti"))) {
        res.globale$quali.var$coord <- res.globale$var$coord[-(1:sum(type.var %in% 
                                                                       c("freq", "quanti"))), ]
        res.globale$quali.var$cos2 <- res.globale$var$cos2[-(1:sum(type.var %in% 
                                                                     c("freq", "quanti"))), ]
        res.globale$quali.var$contrib <- res.globale$var$contrib[-(1:sum(type.var %in% 
                                                                           c("freq", "quanti"))), ]
      }
      else {
  ...
  if (!is.null(ind.quali)) {
    ...
    ind.col <- 0
    for (g in 1:length(group.actif)) {
      cg.partiel <- as.data.frame(matrix(res.globale$call$centre[sum(group.mod[group.actif[1:length(group.actif)]])], 
                                         nrow(barycentre), sum(group.mod[group.actif[1:length(group.actif)]]), 
                                         byrow = TRUE, dimnames = dimnames(barycentre[, 
                                                                                      1:sum(group.mod[group.actif[1:length(group.actif)]]), 
                                                                                      drop = FALSE])))
  ...
  if (graph & (ncp > 1)) {
    ...
    if ("c" %in% type) 
      print(plot.MFA(resultats, choix = "var", habillage = "group", 
                     axes = axes, new.plot = TRUE, shadowtext = TRUE))
    if ("f" %in% type) 
      print(plot.MFA(resultats, choix = "freq", habillage = "group", 
                     axes = axes, new.plot = TRUE))
    print(plot.MFA(resultats, choix = "ind", invisible = "quali", 
                   habillage = "ind", axes = axes, new.plot = TRUE, 
                   col.hab = 1 + 3 * (1:nbre.ind) %in% ind.sup))
    print(plot.MFA(resultats, choix = "group", axes = axes, 
                   new.plot = TRUE))
  }
  return(resultats)
}